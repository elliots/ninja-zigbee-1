...
*/  
  ...
  this._iv = setInterval(function(){
    ...
    {
      self.log.debug('getting settings for '+self.zigbee.type);
      setTimeout(function() {
        self.zigbee.getDeviceState();
      }, 2);
        setTimeout(function() {
    ...
    self.log.debug('waiting for rsp '+self.zigbee.type +' ' +self.zigbee.waitingRsp);
...
NinjaLight.prototype.write = function(data) {
  ...
  {
    self.log.debug('getting settings for inactive '+self.zigbee.type);
    setTimeout(function() {
      self.zigbee.getDeviceState();
    }, 2);
      setTimeout(function() {
